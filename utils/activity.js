...
async function handleActivity(bot, deps, msg) {
    ...
    if (flood.count > 10) { // More than 10 messages in 5 seconds
        ...
        const mention = `<a href="tg://user?id=${userId}">${escapeHTML(msg.from.first_name)}</a>`;
        bot.sendMessage(msg.chat.id, `${mention} is loading blocked for 20 minutes`, { parse_mode: 'HTML' });
