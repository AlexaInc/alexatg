...
function formatLeaderboard(title, items, type, totalMessages, period) {
    ...
    items.forEach((item, index) => {
        ...
        // We use <a href="tg://user?id=${item.userId}">${nameDisplay}</a> here so it functions as a profile link.
        // The disable_notification option in sendMessage prevents it from pinging users.
        text += `${index + 1}. ${statusIcon(index, type)} <a href="tg://user?id=${item.userId}">${nameDisplay}</a> • ${formatNumber(count)}\n`;
    });
