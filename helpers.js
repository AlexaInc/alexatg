...
async function getBuffer(url) {
  try {
    ...
    if (process.env.HFTOKEN && url.includes('hf.space')) {
      options.headers = {
        'Authorization': `Bearer ${process.env.HFTOKEN}`,
        'Referer': 'https://huggingface.co'
      };
      options.proxy = false;
      options.httpAgent = false;
      options.httpsAgent = false;
    }
...
async function downloadImage(url) {
  try {
    const options = { responseType: 'arraybuffer', timeout: 15000, family: 4 };
    if (process.env.HFTOKEN && url.includes('hf.space')) {
      options.headers = {
        'Authorization': `Bearer ${process.env.HFTOKEN}`,
        'Referer': 'https://huggingface.co'
...
async function getUserbotClient() {
  ...
  try {
    ...
    // Check MongoDB for stored userbot session string
    if (!sessionData) {
      try {
        const Session = require('../db/models/session');
        const sessionDoc = await Session.findOne({ key: 'userbot_session_str' });
        if (sessionDoc && sessionDoc.value) {
          sessionData = sessionDoc.value;
          console.log('[getUserbotClient] Loaded userbot session from MongoDB.');
        }
      } catch (dbErr) {
        console.warn('[getUserbotClient] Could not load session from MongoDB:', dbErr.message);
      }
    }

    // Fallback to session.txt
