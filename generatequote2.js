...
async function createImage(firstName, lastName, customemojiid, message, nameColorId, inputImageBuffer, replySender, replyMessage, replysendercolor, messageEntities = []) {
    ...
    // Forced Glyph Subset for special name symbols
    const specialChars = "⎯ᷝ⎯ⷨ͢𝗛𝛬zzυᴰⱽˣ🔥◓𝆺꯭𝅥⎯ͯ";
    const googleFontsUrl = `https://fonts.googleapis.com/css2?family=Noto+Sans:ital,wght@0,700;1,700&family=Noto+Sans+Symbols:wght@700&family=Noto+Sans+Symbols+2:wght@700&family=Noto+Sans+Math:wght@700&family=Noto+Emoji:wght@700&text=${encodeURIComponent(specialChars + " ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789")}&display=block`;
    ...
    const htmlContent = `<html lang="en"><head>
        ...
        .naked-sticker { background: transparent; border-radius: 0; padding: 0; box-shadow: none; min-width: 0; }
