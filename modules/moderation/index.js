module.exports = function (bot, deps) {
    ...
    const getReplacements = async (chatId, member, msg) => {
        const first = escapeHTML(member.first_name || "");
        const last = escapeHTML(member.last_name || "");
        const user = member.username ? "@" + member.username : "no username";
        const userId = member.id;
        const mention = member.username
            ? "@" + member.username
            : `<a href="tg://user?id=${userId}">${first}</a>`;

        let desc = "";
        try {
            const chat = await bot.getChat(chatId);
            desc = escapeHTML(chat.description || "");
        } catch (e) { }
        ...
        return {
            ...
            "{gname}": gname,
            "{group name}": gname,
            "{time}": require('moment-timezone').tz('Asia/Colombo').format('HH:mm:ss'),
            "{date}": require('moment-timezone').tz('Asia/Colombo').format('MMMM Do YYYY'),
            "{day}": require('moment-timezone').tz('Asia/Colombo').format('dddd'),
            "{greating}": getGreeting()
        };
    };

    const applyReplacements = (text, reps) => {
        let result = text || "";
        ...
        try {
            ...
            if (settings?.welcomeEnabled) {
                for (const member of newMembers) {
                    ...
                    const reps = await getReplacements(chatId, member, msg);
                    let text = applyReplacements(settings.welcomeMessage, reps);
    ...
    bot.on('left_chat_member', async (msg) => {
        try {
            ...
            if (settings?.goodbyeEnabled) {
                const member = msg.left_chat_member;
                const reps = await getReplacements(chatId, member, msg);
                let text = applyReplacements(settings.goodbyeMessage, reps);
    ...
    bot.on('message', async (msg) => {
        try {
            ...
            if (antilinkSettings?.enabled) {
                ...
                if (hasLink) {
                    const isAnonAdmin = msg.from.id === 1087968824 || msg.from.username === 'GroupAnonymousBot' || (msg.sender_chat && msg.sender_chat.id === msg.chat.id);

                    let isLinkedChannel = false;
                    if (msg.sender_chat && msg.sender_chat.type === 'channel') {
                        try {
                            const chatInfo = await bot.getChat(chatId);
                            if (chatInfo.linked_chat_id && String(msg.sender_chat.id) === String(chatInfo.linked_chat_id)) {
                                isLinkedChannel = true;
                            }
                        } catch (e) { }
                    }

                    if (isAnonAdmin || isLinkedChannel) {
                        return;
                    }
                    ...
                    if (!isAdmin) {
                        ...
                        if (antilinkSettings.action === 'warn') {
                            ...
                            if (warns.count >= antilinkSettings.warnLimit) {
                                ...
                                bot.sendMessage(chatId, `🚫 <a href="tg://user?id=${userId}">${msg.from.first_name}</a> restricted for ${antilinkSettings.restrictAfterMaxWarns}m (Max Warnings).`, {
                                    parse_mode: 'HTML',
                                    reply_markup: {
                                ...
                                bot.sendMessage(chatId, `⚠️ <a href="tg://user?id=${userId}">${msg.from.first_name}</a>, No links allowed! (${warns.count}/${antilinkSettings.warnLimit})`, {
                                    parse_mode: 'HTML',
                                    reply_markup: {
                                        inline_keyboard: [[
                            ...
                            await bot.restrictChatMember(chatId, userId, { can_send_messages: false, until_date: until });
                            bot.sendMessage(chatId, `🚫 <a href="tg://user?id=${userId}">${msg.from.first_name}</a> restricted for ${antilinkSettings.restrictTime}m for sending a link.`, {
                                parse_mode: 'HTML',
                                reply_markup: {
                                    inline_keyboard: [[{ text: "🔓 Unmute", callback_data: `antiwarn_unmute_${userId}` }]]
                                }
            ...
            if (matchedFilter) {
                ...
                switch (matchedFilter.type) {
                    case 'text': {
                        const reps = await getReplacements(chatId, msg.from, msg);
                        let reptxt = applyReplacements(matchedFilter.reply, reps);

                        // Convert text + entities to HTML since GramJS wrapper doesn't support raw entities option
                        if (matchedFilter.entities && matchedFilter.entities.length > 0) {
                            reptxt = handlers.toHTML(reptxt, matchedFilter.entities);
                        }
                    ...
                    case 'sticker':
                        ...
                        break;
                    case 'image': {
                        const cap = (matchedFilter.entities && matchedFilter.entities.length > 0 && matchedFilter.caption)
                            ? handlers.toHTML(matchedFilter.caption, matchedFilter.entities)
                            : matchedFilter.caption || '';
                        bot.sendPhoto(chatId, matchedFilter.reply, { ...replyTo, caption: cap, parse_mode: 'HTML' });
                        break;
                    }
                    case 'video': {
                        const cap = (matchedFilter.entities && matchedFilter.entities.length > 0 && matchedFilter.caption)
                            ? handlers.toHTML(matchedFilter.caption, matchedFilter.entities)
                            : matchedFilter.caption || '';
                        bot.sendVideo(chatId, matchedFilter.reply, { ...replyTo, caption: cap, parse_mode: 'HTML' });
                        break;
                    }
                    case 'gif': {
                        const cap = (matchedFilter.entities && matchedFilter.entities.length > 0 && matchedFilter.caption)
                            ? handlers.toHTML(matchedFilter.caption, matchedFilter.entities)
                            : matchedFilter.caption || '';
                        bot.sendAnimation(chatId, matchedFilter.reply, { ...replyTo, caption: cap, parse_mode: 'HTML' });
                        break;
                    }
                    case 'audio': {
                        const cap = (matchedFilter.entities && matchedFilter.entities.length > 0 && matchedFilter.caption)
                            ? handlers.toHTML(matchedFilter.caption, matchedFilter.entities)
                            : matchedFilter.caption || '';
                        bot.sendAudio(chatId, matchedFilter.reply, { ...replyTo, caption: cap, parse_mode: 'HTML' });
                        break;
                    }
                    case 'voice': {
                        const cap = (matchedFilter.entities && matchedFilter.entities.length > 0 && matchedFilter.caption)
                            ? handlers.toHTML(matchedFilter.caption, matchedFilter.entities)
                            : matchedFilter.caption || '';
                        bot.sendVoice(chatId, matchedFilter.reply, { ...replyTo, caption: cap, parse_mode: 'HTML' });
                        break;
                    }
                    case 'document': {
                        const cap = (matchedFilter.entities && matchedFilter.entities.length > 0 && matchedFilter.caption)
                            ? handlers.toHTML(matchedFilter.caption, matchedFilter.entities)
                            : matchedFilter.caption || '';
                        bot.sendDocument(chatId, matchedFilter.reply, { ...replyTo, caption: cap, parse_mode: 'HTML' });
                        break;
                    }
                    case 'video_note':
