...
Total: \`${groupChatIds.size + userChatIds.size}\``;
  ...
  bot.onText(/^\/update(?:\s|$|@)/, async (msg) => {
    ...
    try {
      await bot.sendMessage(msg.chat.id, "🚀 Updating code from git... Please wait.");
      ...
      exec('git pull --allow-unrelated-histories', async (err, stdout, stderr) => {
        ...
        bot.sendMessage(msg.chat.id, "✅ Git pull successful. Installing new packages...");

        exec('npm install', async (npmErr, npmStdout, npmStderr) => {
          if (npmErr) {
            return bot.sendMessage(msg.chat.id, `❌ NPM Error: ${npmErr.message}\n${npmStderr}`);
          }

          if (logGrpid) await bot.sendMessage(logGrpid, "✅ Update and package installation successful. Restarting bots...");
  ...
  bot.onText(/^\/fq(?:\s|$|@)/, async (msg) => {
    ...
    try {
      if (hasValidPayload) {
        ...
        if (!botOWNER_IDS.includes(msg.from.id) && botOWNER_IDS.includes(data.sender)) {
      ...
      // Fallback to Userbot if Bot API profile photo failed
      if (!userphoto) {
        const targetId = hasValidPayload ? JSON.parse(content).sender : msg.reply_to_message.from.id;
        const client = await handlers.getUserbotClient();
        if (client) {
          try {
            const entity = await client.getEntity(targetId).catch(() => null);
            if (entity) {
              userphoto = await client.downloadProfilePhoto(entity).catch(() => null);
            }
          } catch (e) { }
    ...
    try {
      ...
      if (action === 'start' || action === 'on') {
        try {
          ...
          if (e.errorMessage === 'GROUPCALL_ALREADY_EXISTS') {
            // VC already active — send join button
            const chatUsername = msg.chat.username;
            const joinOpts = { parse_mode: 'Markdown' };
            if (chatUsername) {
              joinOpts.reply_markup = {
                inline_keyboard: [[{ text: "🎙 Join Video Chat", url: `https://t.me/${chatUsername}?videochat` }]]
              };
            }
            bot.sendMessage(chatId, "ℹ️ A Video Chat is already active in this group.", joinOpts);
          } else {
            throw e;
          }
        }
