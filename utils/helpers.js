...
function getMessageType(msg) {
  if (!msg) return null;
  const result = { type: null, file_id: null, caption: msg.caption || null, entities: msg.caption_entities || msg.entities || null };
  if (msg.photo) { result.type = "image"; result.file_id = msg.photo[msg.photo.length - 1].file_id; }
  else if (msg.video) { result.type = "video"; result.file_id = msg.video.file_id; }
  else if (msg.document) { result.type = "document"; result.file_id = msg.document.file_id; }
  else if (msg.sticker) { result.type = "sticker"; result.file_id = msg.sticker.file_id; }
  else if (msg.animation) { result.type = "gif"; result.file_id = msg.animation.file_id; }
  else if (msg.audio) { result.type = "audio"; result.file_id = msg.audio.file_id; }
  else if (msg.voice) { result.type = "voice"; result.file_id = msg.voice.file_id; }
  else if (msg.video_note) { result.type = "video_note"; result.file_id = msg.video_note.file_id; }
  else if (msg.text) { result.type = "text"; result.file_id = msg.text; }
  else { return null; }
  return result;
}
...
function parseFilterTriggers(commandText) {
  ...
  if (match) {
    ...
    return inside.split(",").map(x => x.trim()).filter(x => x.length > 0);
  }
  return [txt];
}
...
/**
 * Download a buffer from URL - bypasses api.telegram.org 
 * Uses direct HTTP for external URLs only
 */
async function getBuffer(url) {
  if (!url) return null;
  
  // Handle data: URIs (from GramJS wrapper getFileLink)
  ...
  // Handle Buffer passed directly
  if (Buffer.isBuffer(url)) return url;

  // If it's a local file path, read it directly
  const fs = require('fs');
  if (typeof url === 'string' && !url.startsWith('http') && fs.existsSync(url)) {
...
/**
 * Get user profile photo - uses GramJS bot.getChat which returns local file paths
 * NO api.telegram.org/file calls
 */
async function getProfilePhoto(bot, userId) {
  try {
    // Method 1: getUserProfilePhotos (GramJS wrapper returns local paths)
    const photos = await bot.getUserProfilePhotos(userId, { limit: 1 });
    if (photos && photos.photos && photos.photos.length > 0 && photos.photos[0].length > 0) {
      const fileId = photos.photos[0][photos.photos[0].length - 1].file_id;
      // GramJS wrapper's getFileLink downloads via MTProto and returns local path
      return await bot.getFileLink(fileId);
    }
  } catch (e) { /* ignore */ }

  try {
    // Method 2: getChat photo
...
function toHTML(text, entities) {
  ...
  entities.forEach(e => {
    ...
    switch (e.type) {
      ...
      case 'spoiler': start = '<spoiler>'; end = "</spoiler>"; break;
      ...
      case 'expandable_blockquote': start = "<blockquote>"; end = "</blockquote>"; break;
    }
    if (start) {
...
function setCachedUserbotId(id) { cachedUserbotId = String(id); }
function getCachedUserbotId() { return cachedUserbotId; }

/**
 * Resolves a Telegram entity for the userbot client
 */
...
/**
 * Get userbot client (separate user account for advanced operations)
 */
