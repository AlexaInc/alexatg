module.exports = function (bot, deps) {
    ...
    bot.onText(/^\/hangman(?:\s|$|@)/, (msg) => {
        ...
        bot.sendMessage(msg.chat.id, "Please use <code>/newhang</code> to start a new Hangman game!", { parse_mode: 'HTML' });
    });
    ...
    function formatLeaderboardText(game, scope, period, entries) {
        ...
        let text = `🏆 <b>${gameName} Leaderboard</b>\n📍 ${scopeName} · ${periodName}\n\n`;

        if (!entries || entries.length === 0) {
            text += "<i>No scores found for this selection.</i>";
        } else {
            const medals = ['🥇', '🥈', '🥉'];
            entries.forEach((u, i) => {
                const prefix = i < 3 ? medals[i] : `${i + 1}.`;
                text += `${prefix} <a href="tg://user?id=${u.id}">${u.name}</a> — ${u.stat}\n`;
            });
        }
        return text;
    }
