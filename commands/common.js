...
module.exports = function (bot, deps) {
  ...
  bot.onText(/^\/start(?:\s|$|@)/, (msg) => {
    ...
    const startImgSource = process.env.START_IMAGE_URL || START_IMAGE_FILE_ID;

    // Send Photo. Using URL or FILE ID.
    bot.sendPhoto(chatId, startImgSource, {
      caption: caption,
      parse_mode: 'HTML',
  ...
  bot.onText(/^\/aic(?:\s|$|@)/, async (msg) => {
    ...
    try {
      ...
      bot.sendMessage(chatId, `📊 **AI Usage Stats**\nUsed: <code>${currentCount}</code>\nRemaining: <code>${remaining}</code>\nDaily Limit: <code>${dailyLimit}</code>`, { parse_mode: 'HTML' });
    } catch (err) {
  ...
  bot.onText(/^\/q(?:\s|$|@)/, async (msg) => {
    ...
    const chatId = msg.chat.id;
    if (!msg.reply_to_message) {
      return bot.sendMessage(chatId, "Reply to a message with <code>/q</code> or <code>/q [count]</code> to create a quote sticker. Use <code>/q r</code> to include reply context.", { parse_mode: 'HTML' });
    }
    ...
    try {
      ...
      if (client) {
        try {
          ...
          for (let i = 0; i < fetched.length; i++) {
            ...
            // 1. FETCH COLOR VIA BOT API (Matches /fq method)
            let nColorId = 0;
            if (senderIdStr) {
              if (colorCache.has(senderIdStr)) nColorId = colorCache.get(senderIdStr);
              else {
                try {
                  const c = await bot.getChat(senderIdStr).catch(() => null);
                  nColorId = c?.accent_color_id ?? sender?.accentColorId ?? sender?.color?.colorId ?? 0;
                  colorCache.set(senderIdStr, nColorId);
                } catch (e) { }
              }
            }

            // 2. DOWNLOAD PHOTO (PRIORITIZE BOT API WITH HF HEADERS)
            let photo = null;
            if (sender) {
              try {
                const sId = sender.id?.toString() || senderIdStr;
                const pUrl = await getProfilePhoto(bot, sId);
                photo = pUrl ? await downloadImage(pUrl) : null;
                if (!photo) {
            ...
            let entities = [];
            if (m.id === targetMsg.message_id && targetMsg.entities) {
                entities = targetMsg.entities;
                ...
                if (gf) {
                  ...
                  if (ridStr) {
                    if (colorCache.has(ridStr)) rColor = colorCache.get(ridStr);
                      ...
                      try {
                        const rc = await bot.getChat(ridStr).catch(() => null);
                        rColor = rc?.accent_color_id ?? s?.accentColorId ?? s?.color?.colorId ?? 0;
      ...
      if (messagesToProcess.length === 0) {
        ...
        console.log(`[Q Command] Profile Photo url=${photoUrl ? 'YES' : 'NO'} size=${photo?.length || 0}`);
        const chat = await bot.getChat(from.id).catch(() => ({}));
  ...
  bot.onText(/^\/id(?:\s|$|@)/, (msg) => {
    ...
    if (msg.reply_to_message) {
      text += `\nReplied User ID: <code>${msg.reply_to_message.from.id}</code>`;
    }
