...
async function getEmojiStatusBuffer(emojiId) {
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
        try {
            ...
            } else if (!sticker.is_animated && !sticker.is_video) {
                file_id = sticker.file_id;
            }

            if (!file_id) throw new Error("Sticker does not have a renderable static version.");
...
async function renderMessageHTML(text, entities) {
    ...
    for (const entity of sortedEntities) {
        ...
        if (entity.type === 'custom_emoji') {
            const emojiBuffer = await getEmojiStatusBuffer(entity.custom_emoji_id);
            if (emojiBuffer) {
                const base64 = `data:image/png;base64,${emojiBuffer.toString('base64')}`;
                partHtml = `<img src="${base64}" class="message-custom-emoji" />`;
            } else {
            ...
            partHtml = `<i>${escapeHtml(entityText)}</i>`;
        } else if (entity.type === 'code') {
            partHtml = `<code style="background: rgba(255,255,255,0.1); padding: 0 4px; border-radius: 4px;">${escapeHtml(entityText)}</code>`;
        } else {
            partHtml = escapeHtml(entityText);
        }
...
async function createImage(firstName, lastName, customemojiid, message, nameColorId, inputImageBuffer, replySender, replyMessage, replysendercolor, messageEntities = []) {
    ...
    const highlightedMessageHtml = await renderMessageHTML(message, messageEntities);
    // ---------------------------------------------
...
            }
            .name-emoji {
500:                /* No specific rules needed */
501:             }
502: 
503:             /* FIX: Add this new rule for the whitespace span */
504:             .name-whitespace {
505:                 display: inline-block;
506:                 vertical-align: middle;
507:             }
508:             
509:             /* This targets the <img> tags generated by generateNameHtml for the name */
510:             .name-line .name-chunk-image {
511:                  /* height: ${nameImageFontSize}px !important; REMOVED -- THIS FIXES THE SIZE */
512:             }
513: 
514:             .emoji-status { 
515:                 width: ${nameEmojiFontSize * 1.5}px; 
516:                 height: ${nameEmojiFontSize * 1.5}px; 
517:                 margin-left: ${8 * scale}px; /* Keep margin for status emoji */
518:                 vertical-align: middle; 
519:                 border-radius:15%;
520:                 display: inline-block; /* Keep for status emoji alignment */
521:             }
522: 
523:             .message { 
524:                 font-size: ${messageFontSize}px; /* UNIVERSAL SIZE */
525:                 line-height: ${messageLineHeight}; 
526:                 color: #fefcff; 
527:                 word-break: break-word; 
528:                 padding-bottom: ${10 * scale}px;
529:                 text-align: left; 
530:                 /* max-width is now set dynamically in Puppeteer */
531:                 width: 100%; 
532:                 box-sizing: border-box; 
533:             }
534: 
535:             .message-custom-emoji {
536:                 width: 1.2em;
537:                 height: 1.2em;
538:                 vertical-align: middle;
539:                 display: inline-block;
540:                 margin-top: -0.2em;
541:             }
