...
bot.on('message', async (msg) => {
 ...
 // console.log(msg,session)
  ...
  if (isCollecting && (msg.photo || msg.video || msg.animation)) {
    ...
    try {
      ...
      if (session.step === 'qu_mod_media') {
        ...
        bot.sendMessage(chatId, `✅ <b>Media attached:</b> ${mediaType}\n\nReview your question below.`, {
          parse_mode: 'HTML',
          reply_markup: {
            inline_keyboard: [
      ...
      bot.sendMessage(chatId, `✅ <b>Media attached:</b> ${mediaType}\n\nNow send the quiz poll for this question.`, { parse_mode: 'HTML' });
    } catch (err) {
      console.error(err);
  ...
  if (msg.poll && session && session.step === 'collecting_polls') {
    const poll = msg.poll;
    if (poll.type !== 'quiz') {
      return bot.sendMessage(chatId, "❌ Please send a *Quiz* type poll (not a regular poll).", { parse_mode: 'HTML' });
    }
    ...
    const qText = `❓ <b>Question Confirmation</b>\n\n<b>Question:</b> ${escapeHTML(poll.question)}\n<b>Options:</b> \n${poll.options.map((o, i) => `${i === correctOptionId ? '✅' : '❌'} ${escapeHTML(o.text)}`).join('\n')}\n${poll.explanation ? `\n📖 <b>Explanation:</b> ${escapeHTML(poll.explanation)}` : ''}${session.pendingPoll.media ? `\n🖼 <b>Media attached:</b> Yes (${session.pendingPoll.mediaType})` : ''}`;
  ...
  switch (session.step) {
    ...
    case 'description': {
      ...
      bot.sendMessage(chatId,
        `✅ <b>Description set.</b>\n\nStep 3/4 — <b>Time per Question</b>\nHow many seconds should each question stay open? (enter a number between <b>10</b> and <b>600</b>)`,
        { parse_mode: 'HTML' }
      );
      break;
    }
    ...
    case 'open_period': {
      ...
      if (isNaN(secs) || secs < 10 || secs > 600) {
        return bot.sendMessage(chatId, "❌ Please enter a valid number between <b>10</b> and <b>600</b> seconds.", { parse_mode: 'HTML' });
      }
      session.openPeriod = secs;
      session.step = 'mode';
      bot.sendMessage(chatId,
        `✅ <b>${secs} seconds per question.</b>\n\nStep 4/4 — <b>Questions</b>\nHow would you like to provide the questions?`,
        {
          parse_mode: 'HTML',
          reply_markup: {
            inline_keyboard: [
    ...
    case 'edit_name': {
      ...
      delete userSessions[chatId];
      bot.sendMessage(chatId, `✅ Quiz name updated to: *${escapeHTML(text)}*`, {
        parse_mode: 'HTML',
    ...
    case 'q_mod_text': {
      ...
      session.step = 'collecting_polls';
      const q = session.pendingPoll;
      const t = `✅ *Question updated!*\n\n✏️ *Modify Current Question*\n\n<b>Q:</b> ${escapeHTML(q.question)}\n<b>Ans:</b> ${escapeHTML(q.options[q.answer])}\n${q.explanation ? `<b>Exp:</b> ${escapeHTML(q.explanation)}` : ''}`;
    ...
    case 'qu_mod_exp': {
      ...
      const qText = `${updated}\n\n❓ <b>Question Details</b> [${qIdx + 1}]\n\n<b>Q:</b> ${escapeHTML(q.question)}\n<b>Ans:</b> ${escapeHTML(q.options[q.answer])}\n${q.explanation ? `<b>Exp:</b> ${escapeHTML(q.explanation)}` : ''}${q.media ? `\n🖼 <b>Media:</b> (${q.mediaType})` : ''}`;
      bot.sendMessage(chatId, qText, {
        parse_mode: 'HTML',
        reply_markup: {
          inline_keyboard: [
...
bot.on('callback_query', async (query) => {
  ...
  // ── Modify poll during creation ──
    ...
    const q = session.pendingPoll;
    const qText = `❓ <b>Question Confirmation (Modified)</b>\n\n<b>Question:</b> ${escapeHTML(q.question)}\n<b>Options:</b> \n${q.options.map((o, i) => `${i === q.answer ? '✅' : '❌'} ${o}`).join('\n')}\n${q.explanation ? `\n📖 <b>Explanation:</b> ${escapeHTML(q.explanation)}` : ''}`;
  ...
  // ── EDITING FLOW ──
    ...
    if (parts.length === 2) {
      ...
      const text = `🛠 *Edit Quiz: ${escapeHTML(quiz.title)}*\n\n what do you want to change?`;
      await bot.editMessageText(text, {
        chat_id: chatId,
        message_id: messageId,
        parse_mode: 'HTML',
  ...
  if (data.startsWith("qu_info_")) {
    ...
    const qText = `❓ <b>Question Details</b> [${qIdx + 1}]\n\n<b>Q:</b> ${escapeHTML(q.question)}\n<b>Ans:</b> ${escapeHTML(q.options[q.answer])}\n${q.explanation ? `<b>Exp:</b> ${escapeHTML(q.explanation)}` : ''}${q.media ? `\n🖼 <b>Media:</b> (${q.mediaType})` : ''}`;
  ...
  if (data.startsWith("add_qns_")) {
    ...
    await bot.sendMessage(chatId, "📊 <b>Adding Questions</b>\n\nSend new quiz polls one by one. Type /done when finished.", { parse_mode: 'HTML' });
  }
  ...
  if (data.startsWith("q_del_")) {
    ...
    // Refresh the list
    let qText = `❓ *Questions of: ${escapeHTML(quiz.title)}*\n\nDeleted question ${qIdx + 1}.\nClick to delete more:`;
    ...
    await bot.editMessageText(qText, { chat_id: chatId, message_id: messageId, parse_mode: 'HTML', reply_markup: { inline_keyboard: buttons } });
  }
  ...
  if (data.startsWith("list_page_")) {
    ...
    for (const quiz of userQuizzes) {
      keyboard.push([{ text: `📄 ${escapeHTML(quiz.title)}`, callback_data: `info_${quiz.quizId}_${page}` }]);
    }
    const navRow = [];
  ...
  if (data.startsWith("info_")) {
    ...
    const text = `🏷 <b>${escapeHTML(quiz.title)}</b>\n🆔 <code>${quiz.quizId}</code> · ${quiz.questions.length} Qs · ⏱ ${quiz.openPeriod}s/Q${quiz.description ? `\n📄 <i>${escapeHTML(quiz.description)}</i>` : ''}`;
      ...
      reply_markup: {
        inline_keyboard: [
          [
            ...
            { text: "📤 Share", url: `https://t.me/share/url?text=${encodeURIComponent('/quiz ' + qId)}` }
          ],
...
bot.on('document', async (msg) => {
  ...
  if (!fileName.endsWith('.json')) {
    return bot.sendMessage(chatId, "❌ Please upload a valid <code>.json</code> file.");
  }
...
async function saveQuiz(chatId, fromId, questions) {
  ...
  try {
    ...
    const descLine = session.description ? `\n📄 <b>Desc:</b> ${escapeHTML(session.description)}` : '';
    bot.sendMessage(chatId,
      `✅ <b>Quiz Created!</b>\n\n🏷 <b>Name:</b> ${escapeHTML(session.title)}${descLine}\n⏱ <b>Time/Q:</b> ${session.openPeriod}s\n📋 <b>Questions:</b> ${qCount}\n🆔 <b>ID:</b> \`${quizId}\`\n\nUse this in your group:\n<code>/quiz ${quizId}</code>`,
      {
        parse_mode: 'HTML',
        reply_markup: {
          inline_keyboard: [[{ text: "📤 Share", url: `https://t.me/share/url?url=${encodeURIComponent('/quiz ' + quizId)}` }]]
        }
      }
    );
  } catch (e) {
...
async function showQuestionsPage(chatId, messageId, qId, page = 0) {
  ...
  try {
    ...
    let text = `❓ *Questions of: ${escapeHTML(quiz.title)}*\nTotal: ${total} | Page ${page + 1}`;
    const keyboard = [];
        ...
        const results = [
          {
            type: 'article',
            id: quizId,
            title: `🚀 Share: ${escapeHTML(quiz.title)}`,
            description: `Click to send: /quiz ${quizId}`,
            input_message_content: {
              message_text: `<code>/quiz ${quizId}</code>`
            }
          }
        ];
