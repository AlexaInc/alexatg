...
const { generateRankingImage } = require('../utils/ranking_image');
...
module.exports = function (bot, deps) {
    ...
    const handleRanking = async (msg, period = 'overall') => {
        ...
        // 1. Get Top 10 for text leaderboard (filtered by period)
        ...
        // 2. Prepare data for Group Dashboard Image (Consolidated)
        const topOverall = await Activity.find({ chatId }).sort({ 'messages.overall': -1 }).limit(10);
        const groupStats = await GlobalGroupStats.findOne({ chatId });
        const totalMessages = groupStats?.messages?.overall || 0;
        const weekStats = groupStats?.messages?.week || 0;

        // Calculate Global Rank
        const globalRank = await GlobalGroupStats.countDocuments({ 'messages.overall': { $gt: totalMessages } }) + 1;
        ...
        try {
            ...
            if (chat.photo && chat.photo.big_file_id) {
                const fileUrl = await bot.getFileLink(chat.photo.big_file_id);
                const axios = require('axios');

                // Directly fetch from the provided URL (Custom API Root) without any proxy
                ...
                groupAvatarBuffer = Buffer.from(res.data);
            }
        } catch (e) {
            console.error("Error fetching group photo:", e.message);
        }
        ...
        const imageBuffer = await generateRankingImage({
            ...
            globalRank,
            weekStats,
            groupLink: msg.chat.username ? `t.me/${msg.chat.username}` : null,
            topUsers: topOverall,
            groupAvatarBuffer
        });
        ...
        await bot.sendPhoto(msg.chat.id, imageBuffer, {
            ...
            reply_markup: getKeyboard('rank_local', period)
    ...
    const handleCallback = async (query) => {
        ...
        if (query.message.photo) {
            bot.editMessageCaption(text, {
                ...
                console.error("Error editing ranking caption:", err.message);
            });
        } else {
    ...
    const handleFetchHistory = async (msg) => {
        ...
        const statusMsg = await bot.sendMessage(msg.chat.id, `⏳ <b>History sync started...</b>\nLimit: <code>${limit}</code> messages.`, { parse_mode: 'HTML' });
        ...
        try {
            ...
            await bot.editMessageText(`⏳ <b>Syncing...</b>\nResolving member names...`, { chat_id: msg.chat.id, message_id: statusMsg.message_id, parse_mode: 'HTML' }).catch(() => { });
            ...
            while (processedCount < limit) {
                ...
                await bot.editMessageText(`⏳ <b>Syncing (Memory-Safe)...</b>\nProcessed: <code>${processedCount}</code> / <code>${limit}</code>`, {
            ...
            if (processedCount === 0) {
                ...
                return bot.editMessageText("ℹ️ No historical messages found.", { chat_id: msg.chat.id, message_id: statusMsg.message_id, parse_mode: 'HTML' });
            }

            await bot.editMessageText(`✅ <b>Sync Complete!</b>\nTotal: <code>${processedCount}</code> messages.\nYour database is now up to date.`, {
                chat_id: msg.chat.id, message_id: statusMsg.message_id, parse_mode: 'HTML'
            });
