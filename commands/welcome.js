...
ex : {first}: User's first name.
{last}: User's last name.
{user}: User's @username.
{id}: User's Telegram ID.
{mention}: Direct mention (username if available, else a link to the profile).
{desc}: Group description.
{gname}: Group name.
{time}, {date}, {day}, {greating}: Time and date variables.
...
- <code>/cleanwelcome</code>: Toggle cleaning old messages`;
    ...
    bot.onText(/^\/resetwelcome(?:\s|$|@)/i, async (msg) => {
        ...
        await WelcomeSettings.updateOne(
            { groupId: String(chatId) },
            {
                welcomeMessage: "Welcome to {gname}, {first}!",
                welcomeFileId: null,
                welcomeType: 'text'
            },
            { upsert: true }
