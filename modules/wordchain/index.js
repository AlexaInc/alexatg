...
module.exports = function (bot, db, options = {}) {
  const CHAIN_DICTIONARY_FILE = options.dictionaryFile || path.join(__dirname, '../../assets/en.txt');
  ...
  try {
    if (fs.existsSync(CHAIN_DICTIONARY_FILE)) {
      ...
      chainDictionary = content.split(/\s+/).map(w => w.trim().toLowerCase()).filter(w => w.length > 0);
      MAX_POSSIBLE_LENGTH = chainDictionary.reduce((max, word) => Math.max(max, word.length), 0);
      console.log(`Word Chain: Loaded ${chainDictionary.length} words from ${CHAIN_DICTIONARY_FILE}`);
    } else {
  ...
  function handleChainWin(chatId) {
    ...
    if (game.players.length === 1) {
      ...
      bot.sendMessage(chatId, `🏆 <b>VICTORY!</b> 🏆\n\n<b>${winnerName}</b> is the Word Chain Champion!`, { parse_mode: 'HTML' });
  ...
  function initChainGame(chatId) {
    ...
    bot.sendMessage(chatId, `🎮 <b>Word Chain Battle Royale!</b>\n/joinchain to enter. Creator /startchain to begin.`, { parse_mode: 'HTML' });
  }

  // --- COMMANDS ---
  bot.onText(/^\/joinchain(?:\s|$|@)/, (msg) => {
    ...
    bot.sendMessage(chatId, `✅ <b>${msg.from.first_name}</b> joined! (${game.players.length}/${MAX_PLAYERS})`, { parse_mode: 'HTML' });
  });

  bot.onText(/^\/startchain(?:\s|$|@)/, (msg) => {
    const chatId = msg.chat.id;
    const game = chainSessions[chatId];
  ...
  bot.on('message', (msg) => {
    ...
    if (game && game.state === 'playing') {
      ...
      if (text.charAt(0) !== game.lastLetter) return bot.sendMessage(chatId, `❌ Must start with <b>${game.lastLetter.toUpperCase()}</b>`, { parse_mode: 'HTML' });
